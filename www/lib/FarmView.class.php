...
class FarmView extends erazor_macro_Template {
	...
	public function execute() {
		$__b__ = new StringBuf();
		{
			...
			while($__hx__it->hasNext()) {
				...
				$__hx__it2 = call_user_func((FarmView_0($this, $__b__, $eleveur, $pet)));
	...
	function __toString() { return 'FarmView'; }
}
function FarmView_0(&$__hx__this, &$__b__, &$eleveur, &$pet) {
	{
		$_e = $eleveur->get_pets();
		return array(new _hx_lambda(array(&$__b__, &$_e, &$eleveur, &$pet), "FarmView_1"), 'execute');
	}
}
function FarmView_1(&$__b__, &$_e, &$eleveur, &$pet) {
	{
		return $_e->iterator();
	}
}
